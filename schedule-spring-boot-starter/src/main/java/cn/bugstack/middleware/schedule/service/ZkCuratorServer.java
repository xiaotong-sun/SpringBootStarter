package cn.bugstack.middleware.schedule.service;
...
import cn.bugstack.middleware.schedule.task.SchedulingRunnable;
import cn.bugstack.middleware.schedule.util.StrUtil;
...
import static cn.bugstack.middleware.schedule.common.Constants.Global.LINE;
import static cn.bugstack.middleware.schedule.common.Constants.Global.path_root;
...
public class ZkCuratorServer {
    ...
    public static void addTreeCacheListener(final ApplicationContext applicationContext, final CuratorFramework client,
        ...
        treeCache.getListenable().addListener((curatorFramework, event) -> {
            ...
            if ("".equals(json) || json.indexOf("{") != 0 || json.lastIndexOf("}") + 1 != json.length())
                return;
            Instruct instruct = JSON.parseObject(new String(event.getData().getData(), Constants.Global.CHARSET_NAME),
                    Instruct.class);
                ...
                case NODE_UPDATED:
                    if (Constants.Global.ip.equals(instruct.getIp())
                        ...
                        CronTaskRegister cronTaskRegistrar = applicationContext
                                .getBean("bugstack-middlware-schedule-cronTaskRegister", CronTaskRegister.class);
                        ...
                        String path_root_server_ip_clazz_method_status = StrUtil.joinStr(path_root,
                                Constants.Global.LINE, "server", Constants.Global.LINE, instruct.getSchedulerServerId(),
                                Constants.Global.LINE, "ip", LINE, instruct.getIp(), LINE, "clazz", LINE,
                                instruct.getBeanName(), LINE, "method", LINE, instruct.getMethodName(), "/status");
                        // 执行命令
                        Integer status = instruct.getStatus();
                        switch (status) {
                            case 0:
                                cronTaskRegistrar
                                        .removeCronTask(instruct.getBeanName() + "_" + instruct.getMethodName());
                                setData(client, path_root_server_ip_clazz_method_status, "0");
                                logger.info("middleware schedule task stop {} {}", instruct.getBeanName(),
                                        instruct.getMethodName());
                                break;
                            case 1:
                                cronTaskRegistrar.addCronTask(new SchedulingRunnable(scheduleBean,
                                        instruct.getBeanName(), instruct.getMethodName()), instruct.getCron());
                                setData(client, path_root_server_ip_clazz_method_status, "1");
                                logger.info("middleware schedule task start {} {}", instruct.getBeanName(),
                                        instruct.getMethodName());
                                break;
                            case 2:
                                cronTaskRegistrar
                                ...
                                cronTaskRegistrar.addCronTask(new SchedulingRunnable(scheduleBean,
                                        instruct.getBeanName(), instruct.getMethodName()), instruct.getCron());
                                setData(client, path_root_server_ip_clazz_method_status, "1");
                                logger.info("middleware schedule task refresh {} {}", instruct.getBeanName(),
                                        instruct.getMethodName());
                                break;
                        }
                    }
    ...
    public static void appendPersistentData(CuratorFramework client, String path, String data) throws Exception {
        PersistentEphemeralNode node = new PersistentEphemeralNode(client, PersistentEphemeralNode.Mode.EPHEMERAL, path,
                data.getBytes(Constants.Global.CHARSET_NAME));
        node.start();
